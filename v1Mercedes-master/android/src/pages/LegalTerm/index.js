...
class LegalTermPage extends Component {
  ...
  handleClickDecline(e) {
    if (navigator.notification) {
      navigator.notification.alert(
        T.translate("legalTerm.declineAlert"),
        null,
        "Servisim",
        T.translate("notification.general.ok")
      );
    } else {
  ...
  showPermission() {
    cordova.plugins.deneme.permissionCheck("permissionCheck", function(
      response
    ) {
  ...
  askPermission() {
    cordova.plugins.deneme.permissionAsk(
      "permissionAsk",
      function(response) {
  ...
  handleClickAccept2(e) {
    if (navigator.notification) {
      ...
      navigator.notification.confirm(
        ...
        T.translate("notification.push notification.title"),
        [
          T.translate("notification.push notification.reject"),
          T.translate("notification.push notification.accept")
        ]
      );
    }
...
this.iospopup();
  ...
  initLocationService() {
    ...
    navigator.notification.confirm(
      ...
      T.translate("notification.location.title"),
      [
        T.translate("notification.location.reject"),
        T.translate("notification.location.accept")
      ]
    );
  }
  ...
  renderAppDescription() {
    return <AppDescription deviceType={this.props.deviceType} />;
  }

  renderLegalNotice() {
    return <LegalTermNotice deviceType={this.props.deviceType} />;
  }

  renderFOSS() {
    return <LegalTermFOSS deviceType={this.props.deviceType} />;
  }

  render3rdParty() {
    return <ThirdPartyContent deviceType={this.props.deviceType} />;
  }

  renderDataProtection() {
    return <DataProtection deviceType={this.props.deviceType} />;
  }

  renderAppSupport() {
    return <AppSupport deviceType={this.props.deviceType} />;
  }
...
export default connect(
  mapStateToProps,
  { logout, hideLegal, toggleFirsttime }
)(LegalTermPage);
